...
contract RadarEditionsTest is Test {
    ...
    function test_createEdition(uint256 fee) external {
        ...
        (RadarEditions.EditionStatus status, uint256 _fee, uint256 balance, address _owner) = implementation.editions(0);

        assertEq(uint256(status), uint256(RadarEditions.EditionStatus.Created));
        assertEq(_fee, fee);
        assertEq(balance, 0);
